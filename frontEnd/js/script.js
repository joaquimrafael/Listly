...
const ListsScreen = ({ setScreen, setSelectedList }) => {
    ...
    const handleDeleteItem = (listId, productId) => {
        axios.delete(`https://44.214.113.103:8443/api/items?listId=${listId}&productId=${productId}`)
            .then(() => {
    ...
    const handleCreateList = () => {
        ...
        axios.post("https://44.214.113.103:8443/api/lists", { name: listName })
            .then(response => {
    ...
    const handleEditList = () => {
        ...
        axios.put(`https://44.214.113.103:8443/api/lists/${editingList.id}`, { name: listName })
            .then(() => {
                setLists(prevLists =>
        ...
        <div className="container">
            ...
            {!showCreateInput && !editingList && lists.map((list) => (
                <div key={list.id} className="list-item">
                    ...
                    <button onClick={() => {
                        axios.get(`https://44.214.113.103:8443/api/items?listId=${list.id}`)
                            .then(response => {
                                ...
                                if (!items || items.length === 0) {
                                    ...
                                    return axios.delete(`https://44.214.113.103:8443/api/lists/${list.id}`)
                                ...
                                const deleteItemRequests = items.map(item => {
                                    ...
                                    return axios.delete(`https://44.214.113.103:8443/api/items?listId=${list.id}&productId=${item.productId}`);
                                });
                                ...
                                return Promise.all(deleteItemRequests)
                                    ...
                                    .then(() => axios.delete(`https://44.214.113.103:8443/api/lists/${list.id}`))
                                    .then(() => {
...
const ListScreen = ({ setScreen, selectedList, setSelectedProduct }) => {
    ...
    useEffect(() => {
        if (selectedList && selectedList.id) {
            axios.get(`https://44.214.113.103:8443/api/items?listId=${selectedList.id}`)
                .then(response => {
                    const itemList = response.data || [];

                    // Agora buscamos os detalhes dos produtos usando o productId de cada item
                    const productRequests = itemList.map(item =>
                        axios.get(`https://44.214.113.103:8443/api/products/${item.productId}`)
        ...
        axios.get("https://44.214.113.103:8443/api/products")
            .then(response => {
    ...
    const handleAddExistingProduct = () => {
        ...
        axios.post(`https://44.214.113.103:8443/api/items?listId=${selectedList.id}&productId=${selectedProduct.id}&quantity=${parsedQuantity}`)
            .then(() => {
                setItems([...items, { 
                    listId: selectedList.id, 
                    productId: selectedProduct.id, 
    ...
    const handleCreateNewProduct = () => {
        ...
        axios.post("https://44.214.113.103:8443/api/products", productData)
            .then(response => {
                const createdProduct = response.data;

                return axios.post(`https://44.214.113.103:8443/api/items?listId=${selectedList.id}&productId=${createdProduct.id}&quantity=${quantity}`)
                    .then(() => createdProduct);
            })
            .then(createdProduct => {
                setItems([...items, { 
...
const ProductScreen = ({ selectedProduct, setScreen, selectedList }) => {
    ...
    const handleUpdateQuantity = () => {
        ...
        axios.put(`https://44.214.113.103:8443/api/items?listId=${selectedList.id}&productId=${selectedProduct.id}&quantity=${parsedQuantity}`)
            .then(() => {
    ...
    const handleUpdateProduct = () => {
        axios.put(`https://44.214.113.103:8443/api/products/${selectedProduct.id}`, editedProduct)
            .then(() => {
